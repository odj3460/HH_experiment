...
int 		g_baud_rate_2 		= 230400; //921600;//115200;			// default baud-rate serial device...
float		g_force_divider_2 	= 50.0f;			// for more information to refer the RFT sensor manual
float		g_torque_divider_2 	= 2000.0f;			// for more information to refer the RFT sensor manual
...
bool rft_operation_service_2(rft_sensor_serial::rft_operation_2::Request &req, rft_sensor_serial::rft_operation_2::Response &res);
uint8_t rft_response_wait_2(uint8_t opType);
uint8_t rft_response_display_2(uint8_t opType);
uint8_t rft_send_command_2(rft_sensor_serial::rft_operation_2::Request &req);
...
int main(int argc, char** argv)
{
	...
	// initialize COM port
	if( RFT_SENSOR_2.openPort( (char*)g_com_port_2.c_str(), g_baud_rate_2 ) == false )
	{
	...
	RFT_SENSOR_2.m_RFT_IF_PACKET.setDivider(g_force_divider_2, g_torque_divider_2);
	
	//service
	ros::ServiceServer op_service_2 = nh_2.advertiseService("rft_serial_op_service_2", rft_operation_service_2);
	//publisher
	geometry_msgs::WrenchStamped ft_data;
	ros::Publisher rft_publisher_2 = nh_2.advertise<geometry_msgs::WrenchStamped>("RFT_FORCE_2", 1);
	...
	while(ros::ok())
	{
		...
		if( (RFT_SENSOR_2.m_nCurrMode == CMD_FT_CONT) && isSensorOk )
		{
			...
			rft_publisher_2.publish(ft_data);
...
void init_param_2(ros::NodeHandle *nk)
{
	...
	if(!nk->getParam("RFT_COM_PORT", g_com_port_2))
	{
		ROS_WARN("RFT Serial port is not defined, The port is initialized with %s", g_com_port_2.c_str());
		nk->setParam("RFT_COM_PORT", g_com_port_2.c_str()); // set param
		nk->getParam("RFT_COM_PORT", g_com_port_2);    // checking 
	}
	ROS_INFO("RFT Serial port: %s", g_com_port_2.c_str());
	
	// parameter serial port device
	if(!nk->getParam("RFT_COM_BAUD", g_baud_rate_2))
	{
		ROS_WARN("The baud-rate of RFT Serial port is not defined, The baud-rate is initialized with %d", g_baud_rate_2);
		nk->setParam("RFT_COM_BAUD", g_baud_rate_2); // set param
		nk->getParam("RFT_COM_BAUD", g_baud_rate_2);    // checking 
	}
	ROS_INFO("RFT Serial port baud-rate: %d", g_baud_rate_2);
	
	// parameter force/torque divider
	if(!nk->getParam("RFT_FORCE_DEVIDER", g_force_divider_2))
	{
		ROS_WARN("The force divider of RFT sensor is not defined, The force divider is initialized with %f", g_force_divider_2);
		nk->setParam("RFT_FORCE_DEVIDER", g_force_divider_2); // set param
		nk->getParam("RFT_FORCE_DEVIDER", g_force_divider_2);    // checking 
	}
	ROS_INFO("Force Divider of RFT sensor: %f", g_force_divider_2);

	if(!nk->getParam("RFT_TORQUE_DEVIDER", g_torque_divider_2))
	{
		ROS_WARN("The torque divider of RFT sensor is not defined, The torque divider is initialized with %f", g_torque_divider_2);
		nk->setParam("RFT_TORQUE_DEVIDER", g_torque_divider_2); // set param
		nk->getParam("RFT_TORQUE_DEVIDER", g_torque_divider_2);    // checking 
	}
	ROS_INFO("Torque Divider of RFT sensor: %f", g_torque_divider_2);	
	
	// version setting... only to check the s/w version.
	nk->setParam("RFT_SERIAL_SW_VER", ROS_RFT_SERIAL_SW_VER);
}
...
bool rft_operation_service_2(rft_sensor_serial::rft_operation_2::Request &req, rft_sensor_serial::rft_operation_2::Response &res)
{
	...
	std::unique_lock<std::mutex> lock(g_com_port_mutex);
	commandSend = rft_send_command_2( req );
	lock.unlock();
	...
	if( commandSend == RFT_SERVICE_OK )
	{
		...
		{
			res.result = rft_response_wait_2(req.opType);
		}
	}
	else
	{
